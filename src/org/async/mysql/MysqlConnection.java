...
//        logger.setLevel(Level.FINE);
	...
	private void execute(int statementId, int[] types, Object[] params)
        ...
        logger.info("execute statement #" + statementId + " args: "
                + Arrays.toString(params) + "types: "+ Arrays.toString(types) + "nullCount: " + nullCount);
		...
		for (int i = 0; i < params.length; i++) {
			if (params[i] == null) {
                ...
                logger.info("the nullBitsBuffer is " + Arrays.toString(nullBitsBuffer));
			} else {
	...
	public void connect(SelectionKey key) {
		SocketChannel channel = (SocketChannel) key.channel();
        System.out.println("connect is ing");
        try {
			channel.finishConnect();
			key.interestOps(SelectionKey.OP_READ);
			parser.getWaitFor().add(Protocol.HAND_SHAKE);  //设置接下里buffer的解析类型
		} catch (IOException e) {
			if (logger.isLoggable(Level.SEVERE)) {
				logger.log(Level.SEVERE, e.getMessage(), e);
			}
		...
		try {
			...
			while ((read = channel.read(in)) > 0) {
				...
				while (in.remaining() > 0) {
                    System.out.println("parse remain");
					Packet result = parser.parse(in);
//                    System.out.println("the reault is " + result.toString());
                    if (logger.isLoggable(Level.FINER)) {
					...
					if (result != null) {
						if (result instanceof EOF) {
                            System.out.println("the resulr eof is " + result);
