...
public class Parser {
	...
	public Packet parse(ByteBuffer in) {
        System.out.println("the in.remaining is first "+in.remaining());
        while (in.remaining() > 0) {
			if (skip > 0) {
				// TODO do this in on step
                System.out.println("the skip is " + skip);
                in.get();
				skip--;
			} else {
				if (packetSize > 0 && firstByte == null) {
					firstByte = in.array()[in.position()];  //正式数据包的第一个字节
				}
				if (buffer.position() == 0) {
                    System.out.println("the waitFor is" + waitFor.get(0));
                    System.out.println("the dataIdx is" + dataIdx);
                    System.out.println("the message is" + message);
                    itemSize = packetSize < 0 ? 4 : protocol.getPacketMap(
							waitFor.get(0), firstByte)
							.getSize(dataIdx, message);
                    System.out.println("the itemSize is" + itemSize);
				...
				if (itemSize == 0) {
                    System.out.println("Reading Null Terminated String");
                    if (logger.isLoggable(Level.FINER)) {
					...
					System.out.println("the in.remaining is " + in.remaining());
                    System.out.println("the length is " + length);
                    buffer.put(in.array(), in.position(), length);
					packetSize -= length;
					in.position(in.position() + length);
				}
                System.out.println("the buffer.position() is" + buffer.position());
					...
					try {
						if (packetSize < 0) {
							packetSize = (int) Utils.readLong(buffer.array(),
									0, 3);
                            System.out.println("the packetSize is" + packetSize);
                            if (logger.isLoggable(Level.FINER)) {
								logger.finer("Packet size:"
										+ packetSize);
							}
						} else {
                            System.out.println("the firstByte is " + firstByte);
                            PacketMap<Packet> map = protocol.getPacketMap(
									waitFor.get(0), firstByte);
							packet = map.getAssembler().process(dataIdx,
									isNull ? null : buffer, packet, message);  //根据不同的数据块获取包内容信息
	...
	private void readNullTerminated(ByteBuffer in) {  //读取以null结尾的字符串
        System.out.println("the in.remaining() is " + in.remaining());
	...
	private void readLengthCodedString(ByteBuffer in) {
		...
		buffer.put(in.get());
		byte[] ar = buffer.array();
		System.out.println("the buffer size is " + ar.length);
		int f = ar[0] & 0xFF;
		if (f == 251) {
			isNull = true;
			buffer.clear();
